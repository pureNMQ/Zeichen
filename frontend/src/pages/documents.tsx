...
function DocumentEditor({
  ...
  const canvasMetadata = document
    ? `${dirty ? '未保存的更改' : '已保存'} · Markdown 文档`
    : '尚未创建 · Markdown 文档'
  return <form onSubmit={(event) => { event.preventDefault(); void save() }} className="flex min-w-0 flex-1 flex-col"><ErrorNotification message={error} />
    ...
    <main className="flex flex-1 flex-col bg-muted/30"><article data-testid="document-writing-canvas" className="mx-auto flex w-full max-w-3xl flex-1 flex-col px-6 py-14 sm:px-10 sm:py-[4.5rem]"><h1 className="sr-only">{document ? `编辑${document.title}` : `新建${documentLabel(module)}`}</h1><div className="text-xs text-muted-foreground">{canvasBreadcrumb}</div><div className="mt-7"><Label htmlFor="document-title" className="sr-only">标题</Label><Input id="document-title" autoComplete="off" value={title} onChange={(event) => { setTitle(event.target.value); markDirty() }} maxLength={256} required placeholder="无标题" className="h-auto rounded-none border-0 bg-transparent px-0 py-0 text-[2.125rem] font-semibold leading-tight tracking-[-0.045em] shadow-none placeholder:text-muted-foreground/55 focus-visible:ring-0 sm:text-[2.5rem]" /><p className="mt-4 text-xs text-muted-foreground">{canvasMetadata}</p></div>
