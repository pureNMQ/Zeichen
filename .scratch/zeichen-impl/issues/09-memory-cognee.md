...
- memory.* MCP 工具(recall/remember/forget[仅自己的]/list)
- 蒸馏自动触发:会话断开/15 分钟空闲超时/条目阈值(3 条)
- 记忆管理页(项目级,editor+):按 agent 过滤/条目列表(锚点跳转)/删除/清空/修正(cognee update)/token 概览/互通授权配置
- pytest:转接层映射、锚点回指、遗忘范围

产出:记忆全链路(agent 写入→蒸馏→人类管理)。
