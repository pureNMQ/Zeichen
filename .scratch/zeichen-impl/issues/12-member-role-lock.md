...
- **项目成员 Owner 转让(2026-08-04 用户确认并入)**:项目中的 owner 不能通过普通角色下拉直接修改或移除；提供单独的“转让 Owner”流程，仅现 owner 可发起、目标必须是已有项目成员，转让时必须输入发起人当前密码确认。转让成功后目标成为 owner，原 owner 降为 editor；后端与前端均强制该边界并覆盖测试。
...
首用户与本人工作区角色锁定、项目 Owner 密码确认转让、下拉统一定位及相应前后端测试均已完成，用户验收通过。
